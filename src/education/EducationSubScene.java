...
public class EducationSubScene extends SubScene{
    ...
    private String unlockEndingCfg = "";
    private String surveySuffixCfg = "";
    private int codeLengthCfg = 8;
    private Integer surveyPaneIndex = null;
    private int firstModuleIndex = -1;
    ...
    public EducationSubScene(double width, double height, ZipFile questions, JSONObject settings, EducationSubSceneContainer essc) throws ZipException {
        ...
        // NEW: read unlock/survey config
        unlockEndingCfg = jsonobject.optString("unlockEnding", "").trim();
        surveySuffixCfg = jsonobject.optString("surveySuffix", "").trim();
        codeLengthCfg = Math.max(1, jsonobject.optInt("codeLength", 8));

        // unit is locked if an unlock ending is configured and settings doesn't mark it unlocked yet
        unitLocked = !unlockEndingCfg.isEmpty() && !settings.optBoolean("unlocked", false);
        ...
        //Add Contents
        order = new ArrayList<>();
        // Unlock screen first if locked
        if (unitLocked) {
            order.add(new UnlockPane(width, height, unlockEndingCfg, settings, essc.username, this));
        }
        // chooser always present (after unlock if any)
        order.add(choosing_area);
        firstModuleIndex = order.size();
        ...
        // Optional survey code pane near the end
        if (!surveySuffixCfg.isEmpty()) {
            surveyPaneIndex = order.size();
            order.add(new SurveyCodePane(width, height, this, surveySuffixCfg, essc.username, codeLengthCfg, settings));
        }
        ...
        Button end = new Button("go to StartScene");
            ...
            public void handle(MouseEvent event) {
                ...
                container.gotoStart();
                //Set it as completed
                settings.put("completed", true);
                icon.getStyleClass().add("completed");
                // also refresh icon so code appears on it if surveySuffix exists
                refreshIconDecorations();
            }
        });
        if (surveyPaneIndex != null) {
            Button surveyBtn = new Button("Get survey code");
            surveyBtn.setFont(new Font(40));
            surveyBtn.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
                @Override
                public void handle(MouseEvent event) {
                    setPane(surveyPaneIndex);
                }
            });
            ending_root.getChildren().add(surveyBtn);
        }
        ...
        ending_root.getChildren().add(0, finish);
        ...
        if(order.size()<=0) {
            ...
            starting_button.addEventHandler(MouseEvent.MOUSE_CLICKED, new EventHandler<MouseEvent>() {
                ...
                public void handle(MouseEvent event) {
                    ...
                    if (unitLocked) {
                        position = 0; // Unlock pane
                    } else {
                        position = Math.max(1, firstModuleIndex); // first module (skip chooser)
                    }
        ...
        if (unitLocked) {
            root.getChildren().add(order.get(0));
        } else {
        ...
        if(settings.optBoolean("completed", false)) {
            icon.getStyleClass().add("completed");
        }
        refreshIconDecorations();
    }
    ...
    /** Called by UnlockPane after successful unlock. */
    public void startModules() {
        unitLocked = false;
        int idx = firstModuleIndex >= 0 ? firstModuleIndex : 1;
        position = Math.min(idx, order.size() - 1);
        root.getChildren().clear();
        root.getChildren().add(order.get(position));
    }

    /** Rebuilds small lock + code text on the icon according to current settings. */
    public void refreshIconDecorations() {
        // remove any extra children beyond the base title text (index 0)
        while (icon.getChildren().size() > 1) {
            icon.getChildren().remove(icon.getChildren().size() - 1);
        }

        // Locked overlay (if still locked)
        boolean isUnlocked = settings.optBoolean("unlocked", !unlockEndingCfg.isEmpty() ? false : true);
        if (!isUnlocked && !unlockEndingCfg.isEmpty()) {
            Text lock = new Text("🔒");
            lock.setFont(new Font(14));
            StackPane.setAlignment(lock, Pos.TOP_RIGHT);
            icon.getChildren().add(lock);
        }

        // Completed overlay code (if completed and we can compute code)
        boolean isCompleted = settings.optBoolean("completed", false);
        if (isCompleted && !surveySuffixCfg.isEmpty()) {
            String username = settings.optString("username", "").trim();
            if (!username.isEmpty()) {
                String fullHash = SurveyCodePane.sha256Hex(username + surveySuffixCfg);
                String display = fullHash.length() <= codeLengthCfg
                        ? fullHash
                        : fullHash.substring(fullHash.length() - codeLengthCfg);
                Text codeTxt = new Text(display.toUpperCase());
                codeTxt.setFont(new Font(10));
                StackPane.setAlignment(codeTxt, Pos.BOTTOM_CENTER);
                icon.getChildren().add(codeTxt);
            }
        }
    }
